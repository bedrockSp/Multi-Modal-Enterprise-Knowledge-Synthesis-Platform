...
class DecompositionLLMOutput(LLMOutputBase):
    ...
    prior_answer_mode: str = Field(
        default="none",
        description=(
            "How the user is referring to the prior assistant message. Six modes:\n"
            "  - 'none' (default): standalone question or topic continuation.\n"
            "  - 'reasoning': asks the system to think more about the prior message "
            "    (Why? Is that right? Explain that more, How do you know?).\n"
            "  - 'reformat': wants the prior message transformed (table, bullets, "
            "    shorter, translated, just the names). Preserve content, change shape.\n"
            "  - 'correction': user is correcting a claim in the prior message "
            "    ('Actually it was FY24 not FY23', 'No, the CEO is X not Y'). Treat "
            "    the user's correction as authoritative, produce a revised answer.\n"
            "  - 'expansion': wants the prior content EXTENDED to additional scope "
            "    ('Now also include FY25', 'Add Microsoft to that', 'And the costs'). "
            "    Preserve prior content; resolved_query should call out ONLY the new "
            "    scope so retrieval brings back the new chunks.\n"
            "  - 'comparison': wants the prior content RELATED to a different entity/"
            "    period ('How does that compare to Q3?', 'Same for last year'). "
            "    resolved_query should describe the OTHER side so retrieval finds it.\n"
            "Default 'none' when in doubt — wrong mode framing causes more damage than "
            "no framing."
        ),
    )
