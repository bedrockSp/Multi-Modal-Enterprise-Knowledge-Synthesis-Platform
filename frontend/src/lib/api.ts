...
export interface GraphMetadata {
  ...
  thread_id: string;
  version: number;
  status: 'pending' | 'building' | 'ready' | 'failed' | 'absent';
...
export const api = {
  ...
  async buildDocumentGraph(threadId: string): Promise<{ status: string; thread_id: string; message?: string }> {
    const token = getAuthToken();
    const response = await fetch(`${API_URL}/graph/build/${threadId}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || 'Failed to start graph build');
    return data;
  },

  async getDocumentGraph(threadId: string): Promise<DocumentGraph> {
    const token = getAuthToken();
    const response = await fetch(`${API_URL}/graph/${threadId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || 'Failed to load document graph');
    return data as DocumentGraph;
  },

  async getDocumentGraphStatus(threadId: string): Promise<GraphStatus> {
    const token = getAuthToken();
    const response = await fetch(`${API_URL}/graph/${threadId}/status`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || 'Failed to load graph status');
    return data as GraphStatus;
  },

  async listDocumentGraphs(): Promise<GraphListResponse> {
    const token = getAuthToken();
    const response = await fetch(`${API_URL}/graph`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || 'Failed to list document graphs');
    return data as GraphListResponse;
  },

  async deleteDocumentGraph(threadId: string): Promise<{ status: string; thread_id: string }> {
    const token = getAuthToken();
    const response = await fetch(`${API_URL}/graph/${threadId}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || 'Failed to delete document graph');
    return data;
  },
};
